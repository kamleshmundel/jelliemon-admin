    ...
    <>
      ...
      <div className="p-4 min-h-screen text-zinc-200">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => nav(ROUTES.unitAddEdit)}
          ...
          <tbody>
            {units.map((l) => (
              ...
              >
                ...
                <td className="px-4 py-2">
                  {l?.parts?.some((p: any) => p.audio)
                    ? l.parts.map((p: any, i: number) =>
                        p.audio ? (
                          <audio key={i} controls className="mb-2">
                ...
                <td className="px-4 py-2 flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1 bg-red-600 hover:bg-red-700 rounded text-white text-sm transition"
                  >
                    <Trash2 size={16} /> Delete
                  </button>
                  <button
                    onClick={() => nav(ROUTES.unitAddEdit + `?id=${l.id}`)}
                    className="flex items-center gap-1 px-3 py-1 bg-indigo-600 hover:bg-indigo-700 rounded text-white text-sm transition"
                  >
                    <Edit2 size={16} /> Edit
                  </button>
                </td>
        ...
        <Comps.Pagination
          ...
          page={pageObj.page}
          perPage={perPage}
          setPage={(page) => setPageObj((pre) => ({ ...pre, page }))}
        />
      </div>
    </>
  );
